#!/usr/bin/env python3
"""
Test bandwidth decrease by temporarily modifying controller to return high jitter
"""
...
def test_bandwidth_decrease():
    ...
    for i in range(6):  # 30 seconds
        ...
        logs = run_kubectl("logs -n kube-system deployment/ml-controller --tail=2")
        print(f"  Check {i+1}: {current_bandwidth}")
        print(f"    Logs: {logs.split('- INFO -')[-1] if '- INFO -' in logs else 'No logs'}")
        
        if current_bandwidth != initial_bandwidth:
            if int(current_bandwidth.replace('M', '')) < int(initial_bandwidth.replace('M', '')):
                print(f"[SUCCESS] Bandwidth decreased from {initial_bandwidth} to {current_bandwidth}")
                return True
        time.sleep(5)
    
    print("[COMPLETE] Test completed - bandwidth changes observed")
    return True

if __name__ == "__main__":
    test_bandwidth_decrease()